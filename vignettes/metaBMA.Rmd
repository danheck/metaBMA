...
To fit a meta-analysis model, prior distributins on the average effect $d$ and the heterogeneity $\tau$ are required. The package `metaBMA` leaves the user the freedom to choose from several predefined distributions or even define an owen prior density function. The function `prior` facilitates the construction and visual inspection of prior distributions to check whether they meet the prior knowledge about the field of interest.
...
mr <- meta_random(logOR, SE, study, towels,
                  d = prior("norm", c(mean=0, sd=.3), lower=0),
                  tau = prior("t", c(mu=0, sigma=.3, nu=1), lower=0))
mr
