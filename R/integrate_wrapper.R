  ...
  if(method == "bridge"){
    ...
    if(length(lb) == 1){
      ...
      if(data$model == "fixed" && data$prior.d$name != "0"){
        ...
        # wrapper <- function(x) sapply(x, function(xx)
        #   exp(log_posterior(data=data,
        #                     s.row = list(d.fixed=xx))))
      ...
      # integral <- integrate(wrapper,  lower = lb, upper = ub,
      ...
      }else if( max(i1$abs.error, i2$abs.error) / i1$value > .0001){
        warning("Order of integration resulted in different estimates!\n    ",
                i1$value, "  vs.  ", i2$value)
      }else if(i1$abs.error < i2$abs.error){
        integral <- i1
      }else{
        integral <- i2
      }
